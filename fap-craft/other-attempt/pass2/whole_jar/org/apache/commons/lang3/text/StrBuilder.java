...
Builder<String> {
    ...
    public int readFrom(Readable readable) throws IOException {
        ...
        if (readable instanceof Reader) {
            int read;
            Reader r2 = (Reader)readable;
            this.ensureCapacity(this.size + 1);
            while ((read = r2.read(this.buffer, this.size, this.buffer.length - this.size)) != -1) {
                this.size += read;
                this.ensureCapacity(this.size + 1);
            }
        } else if (readable instanceof CharBuffer) {
            CharBuffer cb2 = (CharBuffer)readable;
            int remaining = cb2.remaining();
            this.ensureCapacity(this.size + remaining);
            cb2.get(this.buffer, this.size, remaining);
            this.size += remaining;
        } else {
            while (true) {
                this.ensureCapacity(this.size + 1);
    ...
    public StrBuilder appendln(char ch2) {
        return this.append(ch2).appendNewLine();
    }
    ...
    public StrBuilder deleteAll(char ch2) {
        for (int i2 = 0; i2 < this.size; ++i2) {
            if (this.buffer[i2] != ch2) continue;
            int start = i2;
            while (++i2 < this.size && this.buffer[i2] == ch2) {
            }
            int len = i2 - start;
    ...
    public int lastIndexOf(char ch2) {
        return this.lastIndexOf(ch2, this.size - 1);
    }
    ...
    public boolean equalsIgnoreCase(StrBuilder other) {
        ...
        for (int i2 = this.size - 1; i2 >= 0; --i2) {
            char c12 = thisBuf[i2];
            char c22 = otherBuf[i2];
            if (c12 == c22 || Character.toUpperCase(c12) == Character.toUpperCase(c22)) continue;
            return false;
        }
        return true;
    }
    ...
    extends Writer {
        ...
        public void write(int c10) {
            StrBuilder.this.append((char)c10);
        }
    ...
    extends Reader {
        ...
        @Override
        public int read(char[] b10, int off, int len) {
            if (off < 0 || len < 0 || off > b10.length || off + len > b10.length || off + len < 0) {
            ...
            StrBuilder.this.getChars(this.pos, this.pos + len, b10, off);
            this.pos += len;
            return len;
        }
