...
public class af {
    ...
    public static void a(BufferBuilder bufferBuilder, Vec3d[][] vec3dArray, gv gv2) {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            for (Vec3d vec3d : vec3dArray3 = vec3dArray2[i]) {
                bufferBuilder.pos(vec3d.x, vec3d.y, vec3d.z).tex(0.0, 0.0).color(gv2.a, gv2.d, gv2.c, gv2.b).endVertex();
            }
        }
    }
    ...
    public static void a(Minecraft minecraft, em em2, float f) {
        ...
        GlStateManager.translate((double)0.0, (double)0.01, (double)0.0);
        Entity entity = ((d_)minecraft.getRenderManager().getEntityRenderObject((Entity)em2)).c(em2);
        Vec3d vec3d = em2.Q() ? em2.o() : b6.a(new Vec3d(entity.lastTickPosX, entity.lastTickPosY, entity.lastTickPosZ), entity.getPositionVector(), (double)f);
        Vec3d vec3d2 = b6.a(new Vec3d(entityPlayerSP.lastTickPosX, entityPlayerSP.lastTickPosY, entityPlayerSP.lastTickPosZ), entityPlayerSP.getPositionVector(), (double)f);
        Vec3d vec3d3 = vec3d.subtract(vec3d2);
        vec3d3 = em2.a(vec3d3, f);
        GlStateManager.translate((double)vec3d3.x, (double)vec3d3.y, (double)vec3d3.z);
    }
