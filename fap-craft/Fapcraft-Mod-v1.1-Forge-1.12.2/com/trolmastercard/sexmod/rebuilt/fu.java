...
public class fu {
    ...
    public void a(PlayerInteractEvent.RightClickBlock rightClickBlock) {
        ...
        if (((Boolean)ei2.getDataManager().get(em.G)).booleanValue()) {
        ...
        for (BlockPos blockPos3 : arrayList) {
            ...
            double d2 = this.a(blockPos3.getX(), blockPos3.getY(), blockPos3.getZ(), vec3d.x, vec3d.y, vec3d.z);
            if (!(d2 < (d = this.a(blockPos2.getX(), blockPos2.getY(), blockPos2.getZ(), vec3d.x, vec3d.y, vec3d.z)))) continue;
            blockPos2 = blockPos3;
        }
        if (blockPos2 == null) {
            entityPlayer.sendMessage((ITextComponent)new TextComponentString("Bed is obscured"));
            return;
        }
        entityPlayer.setPosition((double)blockPos2.getX() + 0.5, (double)blockPos2.getY(), (double)blockPos2.getZ() + 0.5);
        if (blockPos.north().equals((Object)blockPos2)) {
            entityPlayer.rotationYaw = 0.0f;
        }
        if (blockPos.east().equals((Object)blockPos2)) {
            entityPlayer.rotationYaw = 90.0f;
        }
        if (blockPos.south().equals((Object)blockPos2)) {
            entityPlayer.rotationYaw = 180.0f;
        }
        if (blockPos.west().equals((Object)blockPos2)) {
            entityPlayer.rotationYaw = -90.0f;
        }
        ...
        ei2.c(new Vec3d((double)blockPos2.getX() + 0.5, (double)((float)blockPos2.getY() + 0.0f), (double)blockPos2.getZ() + 0.5));
        ei2.b(entityPlayer.rotationYaw);
        ei2.getDataManager().set(em.G, (Object)true);
        ei2.u();
    }
    ...
    public void a(PlayerEvent.PlayerRespawnEvent playerRespawnEvent) {
        ...
        Vec3d vec3d = entityPlayer.getPositionVector();
        ei2.dimension = entityPlayer.dimension;
        ei2.setPositionAndUpdate(vec3d.x, vec3d.y, vec3d.z);
        ei2.updateAITasks();
        System.out.println(entityPlayer.world.isAreaLoaded(ei2.getPosition(), 2));
    }
    ...
    public void b(PlayerInteractEvent.EntityInteract entityInteract) {
        block6: {
            ...
            if (ei2 != null) {
                entityPlayerSP.sendStatusMessage((ITextComponent)new TextComponentString("no lesbo yet owo"), true);
                return;
            }
            if (!ei3.l()) {
                return;
            ...
            ei3.b((EntityPlayer)Minecraft.getMinecraft().player);
        }
    }
    ...
    public void a(PlayerInteractEvent.EntityInteract entityInteract) {
        block4: {
            ...
            if (ei3 != null) {
                entityPlayer.sendStatusMessage((ITextComponent)new TextComponentString("no lesbo yet owo"), true);
                return;
            }
    ...
    public void b(PlayerInteractEvent.RightClickBlock rightClickBlock) {
        block8: {
            ...
            ei2.b(entityPlayer.rotationYaw);
            ei2.getDataManager().set(em.G, (Object)true);
            ei2.getDataManager().set(em.D, (Object)0);
            ei2.b(fp.STARTDOGGY);
            if (!rightClickBlock.getWorld().isRemote || !Minecraft.getMinecraft().player.getPersistentID().equals(entityPlayer.getPersistentID())) break block8;
            d3.a(false);
        }
    }
    ...
    public void a(GuiScreenEvent.InitGuiEvent initGuiEvent) {
        ...
        String string = I18n.format((String)(ei2.ah() == 0 ? "action.names.dressup" : "action.names.strip"), (Object[])new Object[0]);
